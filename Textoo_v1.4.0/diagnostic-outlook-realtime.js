// Diagnostic en temps réel pour Outlook - Script à exécuter dans la console
(function() {
  ...
  function analyzeAllElements() {
    ...
    console.log(`\nNombre total d'éléments de texte: ${allTextElements.length}`);
    
    // Chercher spécifiquement les messages de formation
    ...
    console.log(`\nÉléments de formation trouvés: ${formationElements.length}`);
    ...
    if (formationElements.length > 0) {
      ...
      formationElements.forEach((item, index) => {
        ...
        console.log(`Parent Classes: ${item.parentClassName}`);
      });
    } else {
  ...
  // Fonction pour tester la fonction getLastMessageText
  ...
  function runDiagnostic() {
    ...
    if (message) {
      ...
      if (response && (response.includes('formation') || response.includes('planning') || response.includes('confirme'))) {
        ...
        console.log('❌ ERROR: La réponse ne semble pas appropriée au contexte');
        console.log('💡 Conseil: Vérifiez que getLastMessageText() récupère le bon contenu');
      }
    } else {
  ...
  window.testGetLastMessageText = testGetLastMessageText;
