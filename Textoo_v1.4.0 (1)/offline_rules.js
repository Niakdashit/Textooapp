...
(function(){
  ...
  const RULES = [
    ...
    { id:"OFF_PONCTUATION_FIN", description:"Ponctuation de fin par paragraphe",
      test:(t)=>{ return byParagraphs(t,(seg,start)=>{ const trimmed=seg.replace(new RegExp(WSP+"+$",""),""); if(!trimmed) return []; const last=trimmed.slice(-1); if(/[\.?!…]/.test(last)) return []; return [{ offset:start+trimmed.length-1, length:1, message:"Ajoutez une ponctuation de fin.", replacements:[{value:"."}], rule:{id:"OFF_PONCT_FIN"}, type:"style" }]; }); }},
